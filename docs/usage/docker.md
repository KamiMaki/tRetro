...
| tag 版本號 | `penguin88428/retroxpert:v1.1` | **目前最新版本**；每次發佈會進版（v1.1 → v1.2 → …） |
| digest（釘死） | `penguin88428/retroxpert@sha256:7e99e55c…4280d2` | **不可變**，永遠是這一版（用 `@sha256:`，非 `:`） |

目前 `:latest`、`:v1.1`、`@sha256:7e99e55c…` 指向同一個 image。要可重現部署，建議用版本號或 digest，不要只依賴 `latest`。
...
- 目前最新版本：**v1.1**（2026-05-31）。docker 版本號與 `package.json` 的 npm 版本各自獨立。
